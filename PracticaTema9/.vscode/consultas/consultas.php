...
class Consultas
{
    public static function crearTabla()
    {
        ...
        $crearTabla = "CREATE TABLE IF NOT EXISTS usuarios( usuario VARCHAR(30) PRIMARY KEY, contra VARCHAR(1000));";
    ...
    public static function Crear($usuario, $pass)
    {
        ...
        mysqli_stmt_bind_param($stmtInsertar, "ss", $usuario, $pass);
    ...
    public static function RegistroExistente($usuario)
    {
        ...
        $stmusuarios = mysqli_prepare($conexion, $consultausuarios);
        mysqli_stmt_bind_param($stmusuarios, "s", $usuario);
        mysqli_stmt_execute($stmusuarios);

        mysqli_stmt_store_result($stmusuarios);

        if (mysqli_stmt_num_rows($stmusuarios) > 0) {
            $Existe = true;
        }
        mysqli_stmt_close($stmusuarios);
    ...
    public static function Inicio($usuario, $pass)
    {
        ...
        mysqli_stmt_fetch($stmt);
                        //Verifica que la clave introducida sea igual 
        if (password_verify($pass, $claveColumna)) {
            $Existe = true;
        }
    ...
    public static function Update($id, $Dj, $Nacionalidad, $Edad)
    {
        ...
        mysqli_stmt_bind_param($stmt, "sssi", $cancion, $Nacionalidad, $Edad, $id);
        mysqli_stmt_execute($stmt);
